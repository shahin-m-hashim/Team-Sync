...
export default function AddComponent({
  ...
  const validateAndUpload = () => {
    if (renderList === "Task") {
      ...
      requiredRef.current.innerText = "Name is required";
      return;
    } else if (doc.name.length > 25) {
      requiredRef.current.innerText = "Name should be less than 25 characters";
      return;
    }
  ...
  return (
    <div className="absolute inset-0 z-50 size-full backdrop-blur-sm">
      <div
        className={cn(
          renderList !== "Task" ? "w-[400px]" : "w-[470px]",
          showMembers ? "-translate-x-1/3" : "-translate-x-1/2",
          "relative top-1/2 left-1/2 transform -translate-y-1/2 flex items-center gap-y-4 flex-col h-fit bg-[#313338] text-white px-5 py-2"
        )}
      >
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="p-3">
                  ...
                  <input
                    ...
                    value={
                      doc.deadline
                        ? doc.deadline.toISOString().split("T")[0]
                        : ""
                    }
                    placeholder={`Enter Name`}
                    onClick={() => (requiredRef.current.innerText = "")}
